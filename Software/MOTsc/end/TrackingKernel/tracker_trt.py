...
class FairTracker(object):
    def __init__(self, opt, frame_rate=30, enableFP16=False):
        # init trt context
        cuda.init()
        ...
        if enableFP16 == False:
            engine_file_path = '../../weights/fairmot_dyn.trt'
        else:
            engine_file_path = '../../weights/fairmot_dyn_fp16.trt'
    ...
    def update(self, im_blob, image_size, dense_region0=None):
        ...
        with torch.no_grad():
            self.i2shape = {0: (1, 3, im_blob.shape[-2], im_blob.shape[-1])}
